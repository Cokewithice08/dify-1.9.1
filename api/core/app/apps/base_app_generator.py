...
#     encoding: str = "utf-8",
# ) -> str:
#     # 1. 递归处理 obj，过滤 extras.icon.content 中的代理项字符
#     def clean_problem_field(data):
#         if isinstance(data, dict):
#             # 定位到 extras.icon.content 字段，直接替换为合法字符
#             if "extras" in data and "icon" in data["extras"] and "content" in data["extras"]["icon"]:
#                 icon_content = data["extras"]["icon"]["content"]
#                 # 替换代理项字符为普通笑脸（或直接删除）
#                 data["extras"]["icon"]["content"] = icon_content.replace('\ud83d\ude01', '😊')
#             # 递归处理其他字典字段
#             for k, v in data.items():
#                 data[k] = clean_problem_field(v)
#         elif isinstance(data, list):
#             # 递归处理列表元素
#             data = [clean_problem_field(item) for item in data]
#         return data
#
#     # 2. 先清理问题字段，再序列化
#     cleaned_obj = clean_problem_field(obj)
#     # 3. 编码时添加 errors="replace"，兜底处理其他隐藏无效字符
#     json_str = json.dumps(cleaned_obj, ensure_ascii=False)
#     # 关键：encode 时用 replace 自动替换无效字符为 �
#     return json_str.encode(encoding, errors="replace").decode(encoding)
        ...
        match variable_entity.type:
            ...
            case VariableEntityType.FILE_LIST:
                ...
                if not (
                    ...
                    and (all(isinstance(item, dict) for item in value) or all(
                    isinstance(item, File) for item in value))
                ):
            ...
            def gen():
                for message in generator:
                    if isinstance(message, Mapping | dict):
                        ...
                        # yield f"data: {json_dumps(message)}\n\n"
